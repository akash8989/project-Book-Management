...
const updateBook = async function (req, res) {
    try {
        ...
        if (!validate.isValidObjectId(bookId)) {
            res.status(400).send({ status: false, message: `book id is not a valid bookId id` })
            return
        }
...
let deleteBook = async function (req, res) {
    try {
        ...
        let deletedBook = await bookModel.findOneAndUpdate({ _id: bookId, isDeleted: false, deletedAt: null },
            ...
            // first filter then update and new :true returns modifies document rather then original
